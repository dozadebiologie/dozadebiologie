...
De ce se recurge la axe și planuri în descrierea anatomică a corpului uman?|Pentru precizarea poziției segmentelor corpului și a elementelor ce intră în alcătuirea fiecărui organ.
...
const FlashcardChapter: React.FC<FlashcardChapterProps> = ({ user }) => {
  ...
  const chapterTitle = chapters[Number(id) - 1] || "Capitol necunoscut";
            ...
            {isChapterOne ? (
                deck.length > 0 ? (
                    <div className="flex items-center justify-center gap-4 md:gap-12">
                        <button onClick={handlePrev} className="hidden md:flex w-14 h-14 bg-white rounded-full border border-blue-100 items-center justify-center text-blue-500 shadow-sm hover:shadow-md hover:scale-110 transition-all active:scale-90">
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" /></svg>
                        </button>

                        <div className="flex-1 max-w-2xl perspective">
                            <div className={`relative h-[420px] w-full cursor-pointer flip-card-inner preserve-3d ${isFlipped ? 'flipped' : ''}`} onClick={() => setIsFlipped(!isFlipped)}>
                                <div className="absolute inset-0 bg-white p-10 md:p-16 rounded-[40px] shadow-2xl border border-blue-50 flex items-center justify-center backface-hidden">
                                    <p className="text-xl md:text-2xl font-semibold text-slate-800 leading-relaxed">{deck[currentIndex].question}</p>
                                </div>
                                <div className="absolute inset-0 bg-blue-50 p-10 md:p-16 rounded-[40px] shadow-2xl border border-blue-100 flex flex-col items-center justify-center backface-hidden [transform:rotateY(180deg)]">
                                    <p className="text-lg md:text-xl text-blue-900 leading-relaxed mb-10 overflow-y-auto max-h-[220px] no-scrollbar">{deck[currentIndex].answer}</p>
                                    <div className="flex items-center gap-4 w-full mt-auto">
                                        <button onClick={handleLearned} className="flex-1 bg-[#2ecc71] text-white py-4 rounded-2xl font-bold text-sm hover:scale-[1.05] transition-transform shadow-lg shadow-green-100">Am reținut</button>
                                        <button onClick={handleForgot} className="flex-1 bg-[#e67e22] text-white py-4 rounded-2xl font-bold text-sm hover:scale-[1.05] transition-transform shadow-lg shadow-orange-100">Am uitat</button>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <button onClick={handleNext} className="hidden md:flex w-14 h-14 bg-white rounded-full border border-blue-100 items-center justify-center text-blue-500 shadow-sm hover:shadow-md hover:scale-110 transition-all active:scale-90">
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" /></svg>
                        </button>
                    </div>
                ) : (
                    <div className="bg-white p-12 rounded-[40px] shadow-xl border border-gray-100">
                        <p className="text-gray-500 text-lg mb-6 italic">Ai parcurs toate întrebările disponibile momentan.</p>
                        {scheduledCards.length > 0 && <p className="text-blue-500 font-bold text-sm uppercase tracking-widest animate-pulse">Așteptăm reintroducerea celor {scheduledCards.length} întrebări uitate...</p>}
                    </div>
                )
            ) : (
                <div className="bg-white p-12 rounded-[40px] shadow-sm border border-gray-100 max-w-lg mx-auto">
                    ...
                    <p className="text-gray-400 text-lg italic leading-relaxed">Lucrăm la conținutul acestui capitol. Flashcardurile vor fi disponibile în curând!</p>
                </div>
            )}

            <div className="mt-16 flex flex-col items-center gap-8">
                <Link to="/flashcards" className="flex items-center gap-2 text-[10px] font-bold text-gray-400 hover:text-black transition-colors uppercase tracking-[0.2em]">
